...
# install GCC 4.8.2 as Talipot requires a C++11 compiler
...
cmake -DCMAKE_C_COMPILER=/opt/rh/devtoolset-2/root/usr/bin/gcc -DCMAKE_CXX_COMPILER=/opt/rh/devtoolset-2/root/usr/bin/g++ -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$PWD/install -DCMAKE_PREFIX_PATH=$QT_PATH -DPYTHON_EXECUTABLE=/usr/bin/python3.6 -DTALIPOT_USE_CCACHE=$CCACHE -DTALIPOT_BUILD_FOR_APPIMAGE=ON -DTALIPOT_BUILD_TESTS=$RUN_TESTS ..
make -j4 install
...
./appimagetool-$(uname -p).AppImage Talipot.AppDir Talipot-$(sh talipot-config --version)-$(uname -p).AppImage
