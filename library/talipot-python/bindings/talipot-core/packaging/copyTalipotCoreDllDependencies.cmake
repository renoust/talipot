...
GET_PREREQUISITES(${TALIPOT_NATIVE_PYTHON_MODULE} TALIPOT_NATIVE_PYTHON_MODULE_TL_DEPS 1 0 "" ${DIRS})
...
# Store the resolved top level items for the talipot module in an environment variable (as PARENT_SCOPE does not seem
...
FUNCTION(gp_resolve_item_override context item exepath dirs resolved_item_var resolved)
  IF(DEFINED ENV{TALIPOT_NATIVE_PYTHON_MODULE_DEPS})
   SET(ENV{TALIPOT_NATIVE_PYTHON_MODULE_DEPS} "$ENV{TALIPOT_NATIVE_PYTHON_MODULE_DEPS};${${resolved_item_var}}")
  ELSE()
   SET(ENV{TALIPOT_NATIVE_PYTHON_MODULE_DEPS} "${${resolved_item_var}}")
  ENDIF()
ENDFUNCTION()

# Append the top level prerequisites items to the TALIPOT_NATIVE_PYTHON_MODULE_DEPS list
...
FOREACH(DLL_DEP ${TALIPOT_NATIVE_PYTHON_MODULE_TL_DEPS})
  IF(NOT "${DLL_DEP}" MATCHES "^python[0-9][0-9].dll$")
    ...
    GP_RESOLVE_ITEM(${TALIPOT_NATIVE_PYTHON_MODULE} ${DLL_DEP} "" ${DIRS} DLL_DEP_RESOLVED)
  ENDIF()
ENDFOREACH()
...
# Get recursively prerequisites of top level prerequisites and append them to the TALIPOT_NATIVE_PYTHON_MODULE_DEPS list
FOREACH(DLL_DEP $ENV{TALIPOT_NATIVE_PYTHON_MODULE_DEPS})
...
# Finally, resolve all prerequisites for the talipot Python module and copy them to the module native folder
