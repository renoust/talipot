...
static const QString updateVisualizationFunc = "import talipotutils\n"
                                               "\n"
...
static const QString pauseScriptFunc = "import talipotutils\n"
                                       "\n"
                                       "def pauseScript():\n"
...
static const QString runGraphScriptFunc = "import talipotutils\n"
                                          "\n"
...
static QString getDefaultScriptCode(const QString &pythonVersion, Graph *graph) {
  ...
  oss << "from talipot import tlp" << endl << endl;
  ;
...
static QString getTalipotPythonPluginSkeleton(const QString &pluginClassName,
  ...
  textStream << "# and it will be automatically loaded at Talipot startup" << endl << endl;

  textStream << "from talipot import tlp" << endl;
  textStream << "import talipotplugins" << endl << endl;
  ...
  if (pluginType != "Import" && pluginType != "Export") {
    ...
    textStream << "    # The os parameter is an output file stream (initialized by the Talipot GUI"
               << endl;
  ...
  if (pluginGroup.isEmpty()) {
    textStream << "talipotplugins.registerPlugin(\"" << pluginClassName << "\", \"" << pluginName
               << "\", \"" << pluginAuthor << "\", \"" << pluginDate << "\", \"" << pluginInfo
               << "\", \"" << pluginRelease << "\")" << endl;
  } else {
    textStream << "talipotplugins.registerPluginOfGroup(\"" << pluginClassName << "\", \""
  ...
  for (int i = 0; i < _ui->mainScriptsTabWidget->count(); ++i) {
    ...
    if (deleted == -1 || i != deleted) {
      ...
      if (fileName.isEmpty()) {
        ...
        // computer
        // this is usefull for instance when Talipot project and Python files are stored on a git
        // repository
