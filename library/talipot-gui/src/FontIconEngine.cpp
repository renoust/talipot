...
QFont &FontIconEngine::init(const std::string &iconName) {
  // first set code point
  codePoint = IconicFont::getIconCodePoint(iconName);
  // then get font
  auto &&fontFile = IconicFont::getTTFLocation(iconName);
...
FontIconEngine::FontIconEngine(const std::string &iconName) : font(init(iconName)) {}

FontIconEngine::FontIconEngine(const QString &iconName)
    : font(init(QStringToTlpString(iconName))) {}

void FontIconEngine::paint(QPainter *painter, const QRect &rect, QIcon::Mode mode, QIcon::State) {
  painter->save();
