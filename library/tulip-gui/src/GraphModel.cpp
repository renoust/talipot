  ...
  if (graphEv) {
    ...
    if (graphEv->getType() == GraphEvent::TLP_ADD_NODE) {
      ...
      for (auto addedNode : addedNodes) {
        ...
        // element
        int wasDeleted =
            _nodesRemoved ? _elementsToModify.indexOf(qMakePair(addedNode.id, false)) : -1;

        if (wasDeleted == -1) {
          _elementsToModify.push_back(QPair<unsigned int, bool>(addedNode.id, true));
        } else {
          _elementsToModify.remove(wasDeleted);
        }
      }
  ...
  if (dynamic_cast<const GraphEvent *>(&ev) != nullptr) {
    ...
    if (graphEv->getType() == GraphEvent::TLP_ADD_EDGE) {
      ...
      for (auto addedEdge : addedEdges) {
        ...
        // element
        int wasDeleted =
            _edgesRemoved ? _elementsToModify.indexOf(qMakePair(addedEdge.id, false)) : -1;

        if (wasDeleted == -1) {
          _elementsToModify.push_back(QPair<unsigned int, bool>(addedEdge.id, true));
        } else {
          _elementsToModify.remove(wasDeleted);
        }
      }
