...
void MovablePathItem::updatePath() {
  ...
  for (auto &it : _metricToSizeFilteredList) {
    if (it.first < firstLimit) {
      if (state == 0) {
        ...
        pathsPoints[0].push_back(QPoint(
            15 + 15 * it.second, 160 - 160 * (it.first - _minMetric) / (_maxMetric - _minMetric)));
      }
      ...
      if (state == 1) {
        ...
        pathsPoints[1].push_back(
            QPoint(15 + 15. * midValue,
                   160 - 160 * (firstLimit - _minMetric) / (_maxMetric - _minMetric)));
        pathsPoints[1].push_back(QPoint(
            15 + 15. * it.second, 160 - 160 * (it.first - _minMetric) / (_maxMetric - _minMetric)));
        state = 2;
      } else if (state <= 1) {
        pathsPoints[1].push_back(QPoint(
      ...
      if (state == 2) {
        ...
        pathsPoints[2].push_back(
            QPoint(15 + 15. * midValue,
                   160 - 160 * (secondLimit - _minMetric) / (_maxMetric - _minMetric)));
        pathsPoints[2].push_back(QPoint(
            15 + 15. * it.second, 160 - 160 * (it.first - _minMetric) / (_maxMetric - _minMetric)));
        state = 3;
      } else {
        pathsPoints[2].push_back(QPoint(
