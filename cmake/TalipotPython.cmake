SET(TALIPOT_PYTHON_SITE_INSTALL OFF CACHE BOOL "Do you want to install Talipot Python modules in a Python standard module folder on your system ?
...
This should only be used when packaging Talipot for a Linux distribution or MSYS2. [OFF|ON]")
...
"
  ...
  STRING(REPLACE "\n" "" TalipotPythonModulesInstallDir "${TalipotPythonModulesInstallDir}")

ELSE(TALIPOT_PYTHON_SITE_INSTALL)
  SET(TalipotPythonModulesInstallDir ${CMAKE_INSTALL_PREFIX}/${TalipotLibInstallDir}/talipot/python)
...
IF(TALIPOT_ACTIVATE_PYTHON_WHEEL_TARGET)
  STRING(REGEX REPLACE "[^0-9.]" "" TALIPOT_PYTHON_WHEEL_VERSION "${TalipotMajorVersion}.${TalipotMinorVersion}.${TalipotReleaseVersion}")
  ...
  STRING(COMPARE NOTEQUAL "${TALIPOT_PYTHON_TEST_WHEEL_SUFFIX}" "" TALIPOT_GENERATE_TESTPYPI_WHEEL)

  IF(TALIPOT_GENERATE_TESTPYPI_WHEEL)
    SET(TALIPOT_PYTHON_TEST_WHEEL_VERSION ${TALIPOT_PYTHON_WHEEL_VERSION}.${TALIPOT_PYTHON_TEST_WHEEL_SUFFIX})
  ...
  IF(NOT LINUX)
    ...
    IF(NOT WHEEL_OK EQUAL 0)
      MESSAGE("The 'wheel' Python module has to be installed to generate wheels for talipot modules.")
      MESSAGE("You can install it through the 'pip' tool ($ pip install wheel)")
    ENDIF(NOT WHEEL_OK EQUAL 0)
    IF(NOT TWINE_OK EQUAL 0)
      MESSAGE("The 'twine' Python module has to be installed to upload talipot wheels on PyPi.")
  ...
  ELSE(NOT LINUX)
    ...
    ADD_CUSTOM_COMMAND(TARGET wheel POST_BUILD
      ...
      WORKING_DIRECTORY ${TALIPOT_PYTHON_ROOT_FOLDER}
      COMMENT "patching linux talipot-core wheel" VERBATIM)
    ...
    IF(TALIPOT_GENERATE_TESTPYPI_WHEEL)
      ADD_CUSTOM_COMMAND(TARGET test-wheel POST_BUILD
        ...
        COMMAND bash -c "rm ./dist/$(ls -t ./dist/ | head -2 | tail -1)"
        WORKING_DIRECTORY ${TALIPOT_PYTHON_ROOT_FOLDER}
        COMMENT "patching linux talipot-core test wheel" VERBATIM)
    ENDIF(TALIPOT_GENERATE_TESTPYPI_WHEEL)
